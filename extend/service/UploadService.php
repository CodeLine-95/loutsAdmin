...
class UploadService
{
    ...
    protected static function successful($path, \think\File $fileInfo)
    {
        ...
        if(strpos(PUBLIC_PATH,'public') == false){
    ...
    public static function image($fileName, $path, $moveName = true, $autoValidate = true, $root = null, $rule='uniqid')
    {
        ...
        switch ($uploadType){
            case 1 :
                ...
                if(strpos(PUBLIC_PATH,'public')) $imageInfo->dir = str_replace('/public','', $imageInfo->dir);
    ...
    public static function imageStream($key, $content, $path, $root = null){
        ...
        switch ($uploadType){
            case 1 :
                ...
                if(!self::validDir($dir)) return '生成上传目录失败,请检查权限!';
                $name = '.'.DS.$path.DS.$key;
				if(strpos(PUBLIC_PATH,'public')) $name = str_replace('public/','', $name);
                file_put_contents($name,$content);
                $info["code"] = 200;
                $info["name"] = $key;
                //TODO 入口是public需要替换图片路径
                if(strpos(PUBLIC_PATH,'public')) $path = str_replace('public/','', $path);
