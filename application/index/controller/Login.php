...
class Login extends  Controller
{
    ...
    function login(){
        ...
        if($request->isPost()){
            $post = $request->post();
            $db = Db::name('member')
                ->where('member_user',$post['posusername'])
                ->find();
            if(empty($db['member_password'])){
                $this->error('别逗我,用户名不存在！');
            }
            // if(empty($db['shop'])){
            //     $this->error('非店铺用户，不允许登陆收银前台！');
            // }
            if(md5($post['password'])!==$db['member_password']){
                $this->error('密码不正确');
            }else{
                ...
                if(array_key_exists('remember',$post)&&$post['remember']=='on'){
                    ...
                    cookie('shop',$db['member_shop'],3600*24*7);
                }else{
                    $this->clearCookie();
                }
                //session('uid',$db['id']);
                session('posuserid',$db['id']);
                session('posusername',$db['member_name']);
                session('shop',$db['member_shop']);
                UserLog::addLog([
