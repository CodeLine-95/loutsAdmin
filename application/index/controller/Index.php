...
class Index extends Main
{
    public function index()
    {
         $site_config = Db::name('system')
                ...
                $shops = Db::view('member m','member_name')
                ->view('shop s','shop_name','m.member_shop=s.id','LEFT')
                ->where('m.id',session("posuserid"))
                ->find();
                $usinfo = $shops["shop_name"]." | ".$shops["member_name"];
    ...
    public function payMent(){
        ...
        if($request->isAjax()){
            ...
            if($print['print_sn']) printMsg($print['print_sn'],$printContent,1);//打印指令
                ...
                $data['or_status'] = 1;
                $data['or_finish'] = 0;
                $data['or_verify_status'] = 0;
                $data['or_paied'] = 0;
                $data['or_house'] = $house['id'];
                $data['or_contact'] = session('posuserid');//$post['memberid'];
                $data['or_money'] = $post['pay_money'];
                Db::name('order')->where('or_id',$or_id)->update($data);//保存订单
                // foreach($items as $val){
                //         $stocks = Db::name('stocks')->where('goods_id',$val['gd_id'])->where('house_id',$house['id'])->setDec('numbers',$val['it_number']);
                //     }
