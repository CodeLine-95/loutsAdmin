...
class Sales extends  Base
{
    ...
    {//新建销售单
        ...
        if($request->isAjax()){
            $post = $request->post();
            if(empty($post['or_house']))
            $this->error('出库前请选择仓库！');
            $checksales = Stocks::checksales($post['or_id'],$post['or_house']);
            if($checksales > 0)
            $this->error('出库单里有'.$checksales.'款无库存产品，无法出库！');
...
'verify_status' => $verify_status
    ...
    function salestotalJson(){
        ...
        if(!empty($param['member_category'])){
        	 $where[] = ['mb.member_category','=',$param['member_category']];
        }
        ...
        if(!empty($param['or_contact'])){
        	$where[] = ['o.or_contact','=',$param['or_contact']];
        }
    	...
    	->field(['i.*','FORMAT(i.it_price,2) it_price','o.or_user','o.or_verify_user','o.or_verify_status','o.or_verify_time','o.or_create_time','o.or_status','o.or_paied','o.or_finish','m.mcategory_name','g.goodsname','g.sku','g.barcode','g.unit','g.spec','ue.fullname or_user','mb.member_name member_name'])
        ...
        ->join('member mb','o.or_contact=mb.id','LEFT')
        ->join("mcategory m",'m.id=mb.member_category','LEFT')
        ->where("o.or_type='sales' or o.or_type='pos'")
        ->where($where)
        ->order('i.gd_id desc')
        ->paginate($param['limit']);
