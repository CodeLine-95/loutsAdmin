...
//新增采购退货
    ...
    public function editStorage()
    {
        ...
        if($request->isAjax()){
            ...
            if($verify_old==1 && $post['or_verify_status'] == 1){
                //编辑审核后的单据
                $vdata['or_user'] = session("username");
...
function addstorageListJson(){
    ...
    public function storagetotal(){
    	$mcategory = Db::name('mcategory')->where(['status'=>1])->order('id desc')->select();
    	$project = Db::name('project')->where(['status'=>1])->order('id desc')->select();
    	$user_ext = Db::name('user_ext')->order('id desc')->select();
    	return $this->fetch('storagetotal',[
    		'ordertype' => 'procure',
    		'mcategory'=>$mcategory,
    		'project'=>$project,
    		'user_ext'=>$user_ext
    ...
    function storagetotalJson(){
        $param = $this->request->get();
        $where = [];
        if(!empty($param['or_id'])){
            $where[] =[ 'i.or_id','like','%'.$param['or_id'].'%' ];
        }

        if(!empty($param['start'])){
            $where[] = ['o.or_create_time','>=',strtotime($param['start'].' 00:00:00') ];
        }

        if(!empty($param['end'])){
            $where[] = ['o.or_create_time','<=',strtotime($param['end'].' 23:59:59') ];
        }
        
        if(!empty($param['or_depart'])){
        	 $where[] = ['o.or_depart','=',$param['or_depart']];
        }
        
        if(!empty($param['or_project'])){
        	 $where[] = ['o.or_project','=',$param['or_project']];
        }
        
        if(!empty($param['goodsname'])){
        	$where[] = ['g.goodsname','like','%'.$param['goodsname'].'%'];
        }
        
        if(!empty($param['userid'])){
        	$where[] = ['ues.userid','=',$param['userid']];
        }
        
        $where[] = ['o.or_type','=','procure'];
        $where[] = ['o.or_status','=','1'];
        if(empty($param['limit'])){
            $param['limit'] = 10;
        }
        $data = Db::name('item')->alias('i')
    	->field(['i.*','FORMAT(i.it_price,2) it_price','o.or_user','o.or_verify_user','o.or_verify_status','o.or_verify_time','o.or_create_time','o.or_status','o.or_paied','o.or_finish','m.mcategory_name','g.goodsname','g.sku','g.barcode','g.unit','g.spec','ue.fullname or_user','ues.fullname or_verify_user'])
    	->join("order o",'o.or_id=i.or_id','LEFT')
        ->join("goods g",'g.id=i.gd_id','LEFT')
        ->join("mcategory m",'m.id=o.or_depart','LEFT')
        ->join('supplier sp','o.or_contact=sp.id','LEFT')
        ->join('user u','o.or_user=u.username','LEFT')
        ->join('user s','o.or_verify_user=s.username','LEFT')
        ->join('user_ext ue','ue.userid=u.id','LEFT')
        ->join('user_ext ues','ues.userid=s.id','LEFT')
        ->where($where)
        ->order('i.gd_id desc')
        ->paginate($param['limit']);
        $this->genTableJson($data);
    ...
    public function seeOrder(){
        ...
        if($ordertype=="procure")
            ...
            $title ="采购退货单信息";
        $data = Order::alias('o')->join("user u",'o.or_user=u.username','LEFT')
        ->join("user_ext ue",'ue.userid=u.id','LEFT')
        ->join("storehouse s",'o.or_house=s.id','LEFT')
        ->join('supplier sp','o.or_contact=sp.id','LEFT')
        ->join('mcategory m','o.or_depart=m.id','LEFT')
        ->where('o.or_id','=',$or_id)
        ->limit(1)
        ->find();
