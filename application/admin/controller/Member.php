...
//增加客户分类
    ...
    public function memberListJson()
    {
        ...
        $data = Db::view('Member m','id,member_code,member_name,member_category,member_site,member_regtime,member_status')
            ->view('Mcategory mc',['mcategory_name'],'m.member_category = mc.id')
            ->order('m.id desc')
            ->where($where)
    ...
    public function editMember($id)
    {
        ...
        if($request->isAjax()){
            ...
            if(empty($post['member_password'])){
            	Db::name('member')->where('id',$post['id'])->update([
            		'member_name' => $post['member_name'],
            		'member_card' => $post['member_card'],
            		'member_phone' => $post['member_phone'],
            		'member_code' => $post['member_code'],
            		'member_sname' => $post['member_sname'],
            		...
            		'member_user' => $post['member_user'],
            		'member_site' => $post['member_site'],
