...
public function stockslist()
    ...
    public function stocksListJson()
    {
        ...
        if(!empty($param['category'])){
            $where[] = ['g.category','>=',$param['category'] ];
        }

        if(!empty($param['house_id'])){
            $where[] = ['s.house_id','>=',$param['house_id'] ];
        }
        ...
        $data = Db::view('Stocks s','id,numbers,contact')
            ...
            ->view('Goods g',['goodsname,unit,price,spec'],'g.id = s.goods_id','LEFT')
...
{
    $request = $this->request;
    $stock = Db::name('stocks')->find($id);
    $goods = Db::name('goods')->find($stock['goods_id']);
    $data = Db::name('supplier')->where('id',$goods['contact'])->find();
    return $this->fetch('viewSupplier',[
        'data'=>$data
    ]);
}
