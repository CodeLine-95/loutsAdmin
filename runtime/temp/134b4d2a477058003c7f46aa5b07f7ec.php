<?php /*a:2:{s:76:"/www/wwwroot/pt.mbacms.com/application/admin/view/storage/returnnewList.html";i:1580716304;s:64:"/www/wwwroot/pt.mbacms.com/application/admin/view/pub/modal.html";i:1580923564;}*/ ?>
<!doctype html>
        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                ...
                <div class="layui-card-body layui-table-body layui-table-main">
                    <table id="lotus-table"  lay-data="{page:true,limits:[10,20,100],loading:true,toolbar:'#toolbarDemo',url:'returnnewListJson',hash:''}"  class="layui-table layui-hide" lay-filter="lotus-table">
                        <thead>
                        <tr>
                    ...
                    <script type="text/html" id="actionTpl">
                        <button onclick="xadmin.open('查看采购退货订单','seeOrder.html?or_id={{d.or_id}}&ordertype=<?php echo htmlentities($ordertype); ?>',900,600)" class="layui-btn  layui-btn-xs layui-btn-normal">查看</button>
                        {{#  if(d.or_finish==0){  }}
                        <button onclick="xadmin.open('编辑采购退货订单','editReturn.html?or_id={{d.or_id}}',900,600)" class="layui-btn  layui-btn-xs">编辑</button>
                        {{#  } }}
                    ...
                    <script type="text/html" id="finishTpl">
                      ...
                      <span class="layui-badge layui-bg-green">已完成</span>
                        {{# }else{ }}
                        <button class="layui-badge layui-btn layui-btn-sm" onclick="lotus.verifyOrder('{{d.id}}','/admin/pub/finishOrder?ordertype=collection')" title="点击完成时会处理订单付款">未完成</button>
                      {{# } }}
                    </script>
                    <script type="text/html" id="verifyTpl">
                      ...
                      <span class="layui-badge layui-bg-green">已审</span>
                        {{# }else{ }}
                        <button class="layui-btn layui-btn-sm layui-badge" onclick="lotus.verifyOrder('{{d.or_id}}','/admin/pub/verifyOrder?ordertype=dec')" title="点击审核时会处理库存">未审</button>
                      {{# } }}
                    </script>
                </div>
            </div>
